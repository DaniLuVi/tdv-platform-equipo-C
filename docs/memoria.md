...
Durante el ciclo de desarrollo han surgido varios retos técnicos, principalmente derivados de la integración de capas y el uso de las funciones de la versión 3.3.3 de la biblioteca Arcade:
  ...
  - Conflicto de rutas: Uno de los problemas que ocurrió fue que tras realizar commit y push, los cambios no se reflejaban correctamente en los ordenadores de todos los compañeros. Se detectó que el código contenía rutas absolutas vinculadas al directorio local de uno de los compañeros del grupo. Sin embargo, encontramos la solución, modificamos todas las rutas utilizando la librería os.path y también se modificaron las rutas de los archivos correspondientes a la creación del nivel (con Tile Map Editor). De esta manera, ya era posible ejecutarlo en cualquier ordenador.
  
  - Mapas rectangulares dinámicos: Al unificar el nivel del desierto (nivel 4), originalmente programado en un entorno aislado con un formato de celdas de 60 * 60, pudimos ver que los movimientos y las físicas que funcionaban igual en los otros niveles, en este nivel funcionaban de manera totalmente diferente. Ocurría exactamente por la diferencia de que los otros niveles habían sido programados con un formato de celdas de 64 * 64. Para solucionarlo, se realizó un cálculo de una escala sobre la que el mapa se encogería para encajar a la perfección en el mismo cuadro dimensionado que los otros niveles, y de esa forma las físicas funcionarían igual para todos los niveles.

  - Función getattr(): Se ha incorporado la función getattr() de Pythonen varios puntos del código y tiene una explicación. Esta función es empleada como medida de seguridad para el control de concurrencia asíncrona, protegiendo el flujo gráfico con valores por defecto hasta que los archivos de Tiled se procesen por completo en memoria u otros procesos terminen su correcto funcionamiento sin que rompa el programa.
